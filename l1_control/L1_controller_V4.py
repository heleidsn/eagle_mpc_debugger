...
LastEditTime: 2025-03-25 16:31:48
...
class L1AdaptiveControllerNew:
    '''
    description: 尝试使用state_ref来进行控制，利用z_hat和state_ref的差来进行控制
    param {*} self
    return {*}
    '''    
    def __init__(self, dt, model, model_data=None, debug=False, flag_using_z_ref=False):
        self.dt = dt
        ...
        self.debug = debug
        self.a_s = np.array([1, 1, 1, 1, 1, 1, 1, 1, 1]) * 0.001
        self.filter_time_const = np.array([0.0001, 0.0001, 0.0001])  # 注意这里的单位为s， dt = 0.005s
    ...
    def init_controller(self):
        ...
        return {*}
        '''        
        self.z_hat = np.zeros(9)    # 估计的状态向量
        self.z_real = np.zeros(9)    # 参考状态向量
        self.z_ref_vel = np.zeros(9)    # 参考状态向量
        
        self.z_ref_all = np.zeros(19)  # 参考状态向量
        self.current_state = np.zeros(19)  # 当前状态
        
        self.sig_hat = np.zeros(9)  # 估计的扰动
        self.sig_hat_local = np.zeros(9)  # 估计的扰动在机体坐标系下
        self.sig_hat_tracking = np.zeros(9)  # 用来追踪参考状态的扰动
        
        self.z_tilde = np.zeros(9)  # 对于速度状态估计的误差
        self.tracking_error = np.zeros(9)  # 对于速度状态估计的误差

        self.u_ad = np.zeros(9)  # L1控制器的输出
        self.u_mpc = np.zeros(9)  # MPC控制器的输出
        self.u_tracking = np.zeros(9)  # 跟踪控制器的输出
    ...
    def update_z_tilde(self):
        ...
        debug = False
        self.z_tilde = self.z_hat - self.current_state[10:]
        self.z_tilde_ref = self.z_hat - self.z_ref_vel
        
        if debug:
            print('=============================debug for z_tilde=====================================')
            print('current_state: ', round(self.current_state[10:][-2], 2))
            print('self.z_hat: ', round(self.z_hat[-2], 2))
            print('self.z_tilde: ', round(self.z_tilde[-2], 2))
            print('self.z_tilde_ref: ', round(self.z_tilde_ref[-2], 2))
            print('======================================================')
        ...
        self.tracking_error_velocity = self.z_ref_vel - self.current_state[10:]
    ...
    def update_z_hat_euler(self):
        # update predictor
        z_hat_dot = self.get_z_hat_dot()   # in body frame
        
        self.z_hat = self.z_hat + self.dt * z_hat_dot     # in body frame
    
    def update_sig_hat_new(self):
        '''
        ...
        B_bar_inv = M.copy()
        PHI = np.matmul(LA.inv(self.A_s), (self.expm_A_s_dt - np.identity(9)))
        PHI_inv = LA.inv(PHI)
        ...
        sigma_hat_tracking_error = np.matmul(self.A_s, self.current_state[10:] - self.z_ref_vel)  # using rad
        # print('sigma_hat_tracking_error: ', sigma_hat_tracking_error)
        temp = -np.matmul(B_bar_inv, sigma_hat_tracking_error)
        
        # print('temp: ', temp)
        
        weight = np.array([0, 0, 0, 0, 0, 0, 1000, 10000, 100000])
    ...
    def get_u_l1(self):
        ...
        f_l1_lpf = self.low_pass_filter(self.filter_time_const[0], sig_hat_original[:3], self.sig_f_prev)
        self.sig_f_prev = f_l1_lpf
        
        t_l1_lpf = self.low_pass_filter(self.filter_time_const[1], sig_hat_original[3:6], self.sig_t_prev)
        self.sig_t_prev = t_l1_lpf
        
        t_l1_lpf_arm = self.low_pass_filter(self.filter_time_const[2], sig_hat_original[6:], self.sig_t_arm_prev)
...
class L1AdaptiveControllerAll_V2:
    '''
    ...
    def __init__(self, dt, int_model, int_model_data, robot_model, debug=False, flag_using_z_ref=False):
        ...
        # tunning parameters
        self.a_s = np.ones(18) * -0.1 #! Hurwitz matrix
        # self.filter_time_const = np.array([0.001, 0.001, 0.001])  # 注意这里的单位为s， dt = 0.005s
        self.filter_time_const = np.ones(3) * 0.0001  # 注意这里的单位为s， dt = 0.005s  tc = 0.001 alpha = 0.833  tc = 0.005 alpha = 0.5 
    ...
    def init_controller(self):
        ...
        return {*}
        ''' 
        self.current_state = np.zeros(19)  # 当前状态, used for update dynamics
        self.z_ref_all = np.zeros(19)    # 参考状态
        
        self.z_hat = np.zeros(18)     # 估计的状态向量 Euler
        self.z_ref = np.zeros(18)     # 参考状态向量 Euler
        self.z_real = np.zeros(18) # 状态测量值  in rad
        
        self.sig_hat = np.zeros(18)  # 估计的扰动
        
        self.z_tilde = np.zeros(18)  # 对于速度状态估计的误差
        self.z_tilde_ref = np.zeros(18)  # 与参考状态的误差
        self.z_tilde_tracking = np.zeros(18)  # for tracking error
