...
class ROSDataSubscriber(QThread):
    ...
    def mpc_state_callback(self, msg):
        ...
        try:
            ...
            # Extract joint target states (indices 7:9 for joint positions)
            if len(self.mpc_state.state_ref) >= 9:
                self.target_state = self.mpc_state.state_ref[6:8]
                # print(self.mpc_state.state_ref)
                # print(self.target_state)
            
            # Extract drone states from MPC state message
            if len(self.mpc_state.state) >= 17:
                self.drone_states = {
                    'position': np.array(self.mpc_state.state[0:3]),  # x, y, z
                    'velocity': np.array(self.mpc_state.state[9:12]), # vx, vy, vz
                    'orientation': np.array(self.mpc_state.state[3:7]), # qx, qy, qz, qw
                    'angular_velocity': np.array(self.mpc_state.state[12:15]), # wx, wy, wz
                    'timestamp': time.time()
                }
            
            # Extract drone target states
            if len(self.mpc_state.state_ref) >= 17:
                self.drone_target_state = {
                    'position': np.array(self.mpc_state.state_ref[0:3]),  # x, y, z
                    'velocity': np.array(self.mpc_state.state_ref[9:12]), # vx, vy, vz
                    'orientation': np.array(self.mpc_state.state_ref[3:7]), # qx, qy, qz, qw
                    'angular_velocity': np.array(self.mpc_state.state_ref[12:15]) # wx, wy, wz
                }
...
class MPCDisplayGUI(QMainWindow):
    ...
    def create_plots_widget(self, parent_layout):
        ...
        self.drone_orientation_curves = {
            'qx': self.plots['drone_orientation'].plot(pen=pg.mkPen('r', width=3), name='qx'),
            'qy': self.plots['drone_orientation'].plot(pen=pg.mkPen('g', width=3), name='qy'),
            'qz': self.plots['drone_orientation'].plot(pen=pg.mkPen('b', width=3), name='qz'),
            'qw': self.plots['drone_orientation'].plot(pen=pg.mkPen('m', width=3), name='qw')
        }
    ...
    def update_drone_plots(self, time_array):
        ...
        try:
            ...
            self.drone_orientation_curves['qx'].setData(time_array, drone_orientation_array[:, 0])
            self.drone_orientation_curves['qy'].setData(time_array, drone_orientation_array[:, 1])
            self.drone_orientation_curves['qz'].setData(time_array, drone_orientation_array[:, 2])
            self.drone_orientation_curves['qw'].setData(time_array, drone_orientation_array[:, 3])
            
        except Exception as e:
            print(f"Error updating drone plots: {e}")
    
    def update_predicted_plots(self):
        """Update predicted states and controls plots"""
        try:
            ...
            if hasattr(self.ros_subscriber, 'predicted_states') and self.ros_subscriber.predicted_states is not None:
                ...
                if len(predicted_states) > 0:
                    ...
                    if self.display_mode == 'joint':
                        # States format for joints: state[7:9] for joint positions
                        ...
                        # Update predicted velocities for joints
                        ...
                        self.predicted_positions_curves['joint1'].setData(steps, predicted_states[:, 0])  # X position
                        self.predicted_positions_curves['joint2'].setData(steps, predicted_states[:, 1])  # Y position
                        # Could add Z position curve if needed
                        
                        # Update predicted velocities for drone
                        self.predicted_velocities_curves['joint1'].setData(steps, predicted_states[:, 9])   # Vx
                        self.predicted_velocities_curves['joint2'].setData(steps, predicted_states[:, 10])  # Vy
